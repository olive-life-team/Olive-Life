...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Long> {
    ...
    @Query("SELECT m FROM ChatMessage m WHERE m.room.id = :roomId AND m.id > :lastMessageId ORDER BY m.createdAt ASC")
    List<ChatMessage> findMissedMessages(@Param("roomId") Long roomId, @Param("lastMessageId") Long lastMessageId);
}
