...
- 데이터 규모: 5만 건 이상 더미 데이터

### 검색 API 성능 비교

| 항목 | v1 (DB 직접 조회) | v2 (Local Cache) | v3 (Redis Cache) | 비고 |
|:---:|:-------------:|:----------------:|:----------------:|---|
| 평균 응답 시간 |   4.35 s      |    33.03 ms      |     34.67 ms     | v2/v3 비슷함 |
| p(95) 응답 시간 |    11.68 s    |     94.02 ms     |    112.14 ms     |  v2가 소폭 우세 |
| 초당 처리량 (TPS) |  16.27 req/s  |   72.46 req/s    |   75.96 req/s    | v3 우세 |
| 에러율(Fail Rate) |     2.75%     |      0.00%       |      0.00%       |  캐시 도입 시 에러 해결 |
...
- v1은 150명 접속 시 시스템이 붕괴되지만, v3는 아무런 문제 없이 처리함
- 캐시 도입은 선택이 아닌 필수임이 증명됨

#### v2 vs v3: "속도 vs 확장성"

- 속도(Latency): v2(Local)가 v3(Redis)보다 약 18ms(p95 기준) 정도 더 빠르다. 이는 Redis로 향하는 네트워크 통신 시간이 배제되었기 때문이다
- 확장성(Scalability): v2는 서버가 여러 대일 때 각 서버의 캐시 데이터가 달라질 수 있는 위험(Cache Inconsistency)이 있다. 반면, v3는 모든 서버가 하나의 Redis를 바라보므로 데이터 일관성이 완벽히 보장된다
